...
class SpikingNFResNet(nn.Module):
    ...
    def __init__(self, params, block=BasicBlock, layers=[3, 4, 6, 3]):
            ...
            self.OS = params['OS']
            self.FA = params['FA']
            self.bn = params['bn']  # whether use batch normalization
            self.bias = params['bias']
            self.tau = params['tau']
            self.thresh = params['threshold']
            self.surrogate_function = params['surrogate_function']
            self.is_print_stats = params['print_stats']
            ...
            elif len(self.replace_stride_with_dilation) != 3:
            ...
            expected_var = 1.0
            self.layer1, expected_var = self._make_layer(block, 64, layers[0], alpha=self.alpha, var=expected_var, neuron=neuron,  tau=self.tau, v_threshold=self.thresh,
            ...
            self.layer2, expected_var = self._make_layer(block, 128, layers[1], stride=2, alpha=self.alpha, var=expected_var,
                                                         dilate=replace_stride_with_dilation[0], neuron=neuron,  tau=self.tau, v_threshold=self.thresh,
                                         surrogate_function=self.surrogate_function)
            self.layer3, expected_var = self._make_layer(block, 256, layers[2], stride=2, alpha=self.alpha, var=expected_var,
                                                         dilate=replace_stride_with_dilation[1], neuron=neuron,  tau=self.tau, v_threshold=self.thresh,
                                         surrogate_function=self.surrogate_function)
            self.layer4, expected_var = self._make_layer(block, 512, layers[3], stride=2, alpha=self.alpha, var=expected_var,
                                                         dilate=replace_stride_with_dilation[2], neuron=neuron,  tau=self.tau, v_threshold=self.thresh,
                                         surrogate_function=self.surrogate_function)
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

            self.fc = nn.Linear(512 * block.expansion, self.num_classes, bias=self.bias)
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer1(x)
        print(self.layer1[0].sn1.pre_trace)
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)
...
# def _spiking_resnet(arch, block, layers, params):
#     model = SpikingNFResNet(block, layers, params)
#
#     return model
#
#
...
# def spiking_nfresnet34(params):
#     return _spiking_resnet('resnet34', BasicBlock, [3, 4, 6, 3], params)
#
#
#

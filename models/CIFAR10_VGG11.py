...
class SpikingVGGBN(nn.Module):
    def __init__(self, os, vgg_name, neuron: callable = None, dropout=0.0, num_classes=10, **kwargs):
        super(SpikingVGGBN, self).__init__()
        self.whether_bias = True
        self.init_channels = kwargs.get('c_in', 3)
        ...
        if os is True:
            self.classifier = nn.Sequential(nn.Linear(512 * 1 * 1, num_classes))
        else:
            self.classifier = nn.Sequential(
                neuron(layer=nn.Linear(512 * 1 * 1, num_classes), bn=None, **kwargs),
            )
    ...
    def _make_layers(self, cfg, dropout, neuron, **kwargs):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers.append(
                    neuron(layer=nn.Conv2d(self.init_channels, x, kernel_size=3, padding=1, bias=self.whether_bias),
                           bn=nn.BatchNorm2d(x), **kwargs))
...
def spiking_vgg11_bn(os, neuron: callable = None, num_classes=10, neuron_dropout=0.0, **kwargs):
    return SpikingVGGBN(os, 'VGG11', neuron=neuron, dropout=neuron_dropout, num_classes=num_classes, **kwargs)
